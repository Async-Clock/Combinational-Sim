...
fvec=fopen(argv[3], "r");
Total = ReadVec(fvec, vector); //Total keeps track of the total number of patterns to be simulated
fclose(fvec);
...
for (a; a < Total; a++)
{
  simulation(graph, vector[a], fres, Max); //simulates 1 input pattern at a time
}
