...
while(!feof(fisc)){
  ...
  switch (graph[id].Type)  {
    ...
    case INPT  : break;
    case AND   : for(i=1;i<=Fin;i++) {
		   fscanf(fisc, "%d", &fid);
                   InsertList(&graph[id].Fin,fid);  
                   InsertList(&graph[fid].Fot,id); }  
		   fscanf(fisc,"\n");  break;
    case NAND  : for(i=1;i<=Fin;i++) {
		   fscanf(fisc, "%d", &fid);
                   InsertList(&graph[id].Fin,fid);  
                   InsertList(&graph[fid].Fot,id); }  
		   fscanf(fisc,"\n");  break;
    case OR    : for(i=1;i<=Fin;i++) {
		   fscanf(fisc, "%d", &fid);
                   InsertList(&graph[id].Fin,fid);  
                   InsertList(&graph[fid].Fot,id); }  
		   fscanf(fisc,"\n");  break;
    case NOR   : for(i=1;i<=Fin;i++) {
		   fscanf(fisc, "%d", &fid);
                   InsertList(&graph[id].Fin,fid);  
                   InsertList(&graph[fid].Fot,id); }  
		   fscanf(fisc,"\n");  break;
    case XOR   : for(i=1;i<=Fin;i++) {
		   fscanf(fisc, "%d", &fid);
                   InsertList(&graph[id].Fin,fid);  
                   InsertList(&graph[fid].Fot,id); }  
		   fscanf(fisc,"\n");  break;
...
int ReadVec(FILE *fvec, PATTERN *vector)
{
  ...
  while (!feof(fvec))
  {
    ...
    if (holder == '\n')
    {
      vector[j].length = k; //used for printing out primary inputs
      j++;
      k = 0;
    }
    else if (holder != 'x' && holder != 'X')
    {
      vector[j].piv[k] = holder - '0';
      k++;
    }
    else
    {
  ...
  return j; //returns number of input vectors
}
/****************************************************************************************************************************/
